...
export default function DirectIncomeSavingsManager({ isOpen, onClose }) {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // Récupération des règles de revenus directs et des enveloppes d'épargne cibles
  ...
  const handleAddRule = async (e) => {
    ...
    try {
      ...
      // Insertion dans la table revenus_directs_epargne
        ...
        <div className="mt-4">
          ...
          <div className="list-wrapper" style={{maxHeight: '200px'}}>
            {incomeRules.length === 0 ? (
              <p className="text-[12px] opacity-50 text-center py-4">Aucun revenu direct configuré</p>
            ) : (
              incomeRules.map(r => (
                <div key={r.id} className="list-item">
                  <div className="flex flex-col">
                    <span className="font-medium">{r.nom}</span>
                    <span className="text-[10px] opacity-40">Cible : {r.sous_comptes_epargne?.nom || 'Enveloppe inconnue'}</span>
                  </div>
